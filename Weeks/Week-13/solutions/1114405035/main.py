...
# 3x5 Tiny Font for English Labels
FONT_3X5 = {
...
# Keep track of last successful readings to persist on OLED in case of failure
last_temp = 25.0
last_hum = 50.0

frame = 0
while True:
    # A. Read DHT22 sensor every 2 seconds (8 frames * 250ms = 2000ms)
    if frame % 8 == 0:
        try:
            dht_sensor.measure()
    ...
    # 1. Top Sensor Status & Student ID Section (Y: 0 to 36)
    draw_tiny_text(oled, "ID: 1114405035", 2, 2)
    draw_tiny_text(oled, "TEMP: {:.1f} C".format(last_temp), 2, 12)
    draw_tiny_text(oled, "HUM:  {:.1f} %".format(last_hum), 2, 22)
    oled.hline(0, 32, 128, 1) # divider line

    # 2. English Info Area (Y: 38 to 54)
    ...
    # 3. Animated Dragon Ball (pulsing star size)
    star_size = 11 + (frame % 3) * 2 # Pulsing star size (11, 13, 15)
    draw_circle(oled, center_x, center_y, 35, 1)
    draw_circle(oled, center_x, center_y, 32, 1)
    draw_star(oled, center_x, center_y, star_size, 1)

    # 4. Dancing Chinese Text Block
    draw_dancing_chinese(oled, frame)

    oled.show()

    # C. Increment frame & delay
    frame = (frame + 1) % 24  # least common multiple of 3 (star size pulse) and 8 (DHT22 reading)
    time.sleep_ms(250)
